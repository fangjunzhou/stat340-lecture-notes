...
---
title: "STAT340 Lecture 01: Random Variables"
...
```{r}
x <- seq(-4,4,0.1); f <- dnorm(x, 0,1); # eval density at x values, mean mu=0, standard deviation sigma=1.
...
pp <- ggplot( df, aes(x=x, y=density, color=mean) ) + geom_line( size=1)
pp
```
...
pp <- ggplot( df, aes(x=x, y=density, color=variance) ) + geom_line( size=1)
pp
```
